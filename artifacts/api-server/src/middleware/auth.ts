...
const PUBLIC_PREFIXES = ["/orcamentos/", "/export/"];
...
export function requireAuth(req: Request, res: Response, next: NextFunction) {
  ...
  const isPdfOrExport = PUBLIC_PREFIXES.some((prefix) => req.path.startsWith(prefix));

  if (isPublic || isPdfOrExport) return next();
