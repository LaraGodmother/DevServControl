...
router.get("/services/:id", async (req, res) => {
  try {
    const [service] = await db.select().from(servicesTable).where(eq(servicesTable.id, Number(req.params.id))).limit(1);
...
router.post("/services", async (req: any, res) => {
  try {
    ...
    if (!name || !description || basePrice == null) return res.status(400).json({ error: "name, description e basePrice são obrigatórios." });
...
router.patch("/services/:id", async (req: any, res) => {
  try {
    if (req.jwtUser?.role !== "admin") return res.status(403).json({ error: "Acesso negado." });
    const { name, description, basePrice, profitMargin, rules, active } = req.body;
