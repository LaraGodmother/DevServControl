...
router.post("/appointments", async (req, res) => {
  try {
    ...
    if (!clientId || !serviceId || !date || !time) return res.status(400).json({ error: "clientId, serviceId, date e time são obrigatórios." });
...
router.patch("/appointments/:id", async (req, res) => {
  try {
    ...
    const [appt] = await db.update(appointmentsTable).set({
      ...
      ...(date !== undefined && { date }),
      ...(time !== undefined && { time }),
    }).where(eq(appointmentsTable.id, Number(req.params.id))).returning();
